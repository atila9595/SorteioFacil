    ...
    .map(async (row) => {
      if (row.get("Id") === String(comprador.id)) {
        ...
        try {
          ...
          const messagem =
            comprador.type === "del"
              ? "Sucesso ao deletar comprador."
              : comprador.type === "alt"
              ? "Sucesso ao alterar comprador."
              : comprador.type === "add"
              ? "Sucesso ao adicionar comprador."
              : "Tipo de operação desconhecida.";
          console.log(messagem);
        } catch (error) {
          const messagem =
            comprador.type === "del"
              ? `Erro ao deletar comprador: ${error}`
              : comprador.type === "alt"
              ? `Erro ao alterar comprador: ${error}`
              : comprador.type === "add"
              ? `Erro ao adicionar comprador: ${error}`
              : "Tipo de operação desconhecida.";
          console.log(messagem);
        }
      } else {
...
const addCompradorTosheet = async (comprador) => {
  ...
  const dataAtual = new Date();
            const formatoData = `${dataAtual.getDate()}/${
